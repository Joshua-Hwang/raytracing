...
public:
  Sphere() {}
  Sphere(Point3 cen0, Point3 cen1, double time0, double time1, double r,
         std::shared_ptr<Material> m)
      : center0(cen0), center1(cen1), time0(time0), time1(time1), radius(r),
        mat_ptr(m){};
  Sphere(Point3 cen, double r, std::shared_ptr<Material> m)
      : Sphere(cen, cen, 0.0, 1.0, r, m) {}
...
inline bool Sphere::hit(const Ray &r, double t_min, double t_max,
  ...
  // (discriminant == 0) then we have a single solution, the ray grazes the
  // sphere (discriminant > 0) then we two solution and the ray entered the
  // sphere
  if (discriminant < 0) {
    return false;
  }
